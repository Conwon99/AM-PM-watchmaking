import { FAQAccordion } from "@/sections/FAQSection/components/FAQAccordion";

interface FAQSectionProps {
  /** When true, shows a compact section title (e.g. on home). When false, shows page-style heading (e.g. on FAQ page). */
  compact?: boolean;
}

export const FAQSection = ({ compact = true }: FAQSectionProps) => {
  return (
        ...
        <div className="items-start box-border caret-transparent gap-x-7 flex flex-col justify-between gap-y-7 md:gap-x-10 md:flex-row md:gap-y-10">
          <div className="static items-start box-border caret-transparent gap-x-4 flex flex-col max-w-none gap-y-4 top-[100px] md:sticky md:max-w-[332px]">
            ...
            {compact && (
              <a
                href="/faq"
                className="text-cta font-medium text-[15px] leading-6 hover:text-yellow-700 md:text-base"
              >
                View all on FAQ page →
              </a>
            )}
          </div>
          <div className="box-border caret-transparent flex-1 max-w-[752px]">
            <FAQAccordion />
          </div>
        </div>
      </div>
